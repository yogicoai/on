...
| `ads-campaign` | 광고 일별×캠페인 — 광고비·노출·클릭·전환·전환매출·장바구니 (CSV 업로드분, 7개 캠페인) | 필요 |
...
**ads-campaign** — `date` 일자 · `campaign` 캠페인명 · `spend` 광고비(원) · `imp` 노출 · `clk` 클릭 · `conv` 전환수 · `convValue` 전환매출(원) · `cart` 장바구니수 · `cartValue` 장바구니금액(원) *(CSV 업로드분 — 네이버 ADVoost·커뮤니케이션애드 위주 7개 캠페인, 수기 갱신이라 최신 며칠은 비어있을 수 있음. 매체 전체 광고비는 `ads`)*
...
| 광고(매체) | 매일 오전 9시 30분경 |
| 광고 캠페인(ads-campaign) | CSV 수동 업로드 (부정기 — 업로드 시점 기준) |
...
에이전트·외부 시스템에서 이 API에 연동할 때 자주 묻는 항목입니다.

**Q. 토큰 권한은?**
읽기 전용(조회 전용)입니다. GET만 있고 데이터 변경 경로가 없습니다. 발급 토큰도 이 데이터 조회만 허용합니다.

**Q. 인증 헤더 형식은?**
`Authorization: Bearer <토큰>` 입니다. (`x-api-key` 아님)

**Q. 엔드포인트 목록은?**
- `GET /api/export/catalog` — 사용 가능한 데이터셋·파라미터를 스스로 설명하는 목록. **연동 시 여기부터 호출**하면 데이터셋 구성을 코드로 파악할 수 있습니다.
- `GET /api/export?dataset=<이름>&period=<기간>&format=json|csv` — 데이터 조회.
- 데이터셋은 위 **1번 표**의 이름을 사용합니다.

**Q. 광고를 캠페인·상품 단위로 주는 경로가 있나요?**
- **캠페인 단위: 있습니다** → `dataset=ads-campaign` (일별×캠페인, 광고비·노출·클릭·전환·전환매출·장바구니). 단 CSV 업로드분이라 네이버 ADVoost·커뮤니케이션애드 위주 **7개 캠페인**이고 수기 갱신입니다.
- **매체 단위**는 `dataset=ads` (네이버/메타/크리테오/카카오 등 매체×일).
- **상품(productCode) 단위: 없습니다.** 캠페인명에 제품/라인이 녹아있을 뿐(예: "ADVoost 쇼핑_샐리필") 별도 상품코드 필드는 제공하지 않습니다. 상품 단위 광고가 필요하면 광고 수집 파이프라인 확장이 필요한 별도 작업이라 협의가 필요합니다.

**Q. 호출 제한(rate limit)은?**
- 현재 **분당/시간당 호출 제한은 없습니다.** 접근 제어는 토큰 인증과 **요청당 최대 50,000행** 상한뿐입니다.
- 서버는 Vercel 서버리스로 **요청당 실행 최대 60초**입니다(호출 빈도 제한이 아니라 단일 요청 시간 제한). 큰 기간은 나눠 호출하세요.
- 상식적인 범위(예: 분당 수십 회 이하)면 충분합니다. 시간당 1회 조회는 전혀 문제없습니다. 향후 트래픽이 커지면 rate limit을 도입할 수 있고, 그 경우 사전 공지합니다.
