...
async function dbConn() {
  ...
  return (await g.promise).db(DB_NAME);
}
async function coll() { return (await dbConn()).collection(COLL); }

// 캠페인 단위 광고 — kv.uploaded-campaigns(CSV 업로드분). 일별×캠페인 imp/clk/spend/전환(purch)/전환매출(purchVal).
//   ⚠️ daily_stats(매체×일, 자동 API 적재)와 별개 소스: CSV 수동 업로드분이라 캠페인 7종·수기 갱신.
//   date 포맷은 'YYYY-MM-DD'(daily_stats의 YYYYMMDD와 다름).
async function campaigns(start, end) {
  const kv = (await dbConn()).collection('kv');
  const doc = await kv.findOne({ _id: 'uploaded-campaigns' });
  const all = Object.values((doc && doc.value) || {});
  const s = String(start || '').slice(0, 10), e = String(end || '').slice(0, 10);
  return all
    .filter((r) => r && r.date >= s && r.date <= e)
    .map((r) => ({
      date: r.date, campaign: r.campaign,
      spend: N(r.spend), imp: N(r.imp), clk: N(r.clk),
      conv: N(r.purch), convValue: N(r.purchVal), cart: N(r.cart), cartValue: N(r.cartVal),
    }))
    .sort((a, b) => a.date.localeCompare(b.date) || String(a.campaign).localeCompare(String(b.campaign)));
}
...
module.exports = { configured, efficiency, dailyTrend, rows, campaigns, canonPlatform, vendorOf };
