...
async function gatherContext(start, end) {
  ...
  return {
    ...
    선택구간_프로모션성과: ranged
      // 기간은 '집계에 쓴 구간'(프로모션 기간 ∩ 선택 구간)을 보여준다 — 원본 기간만 주면 AI가 기간 전체 매출로 오해한다.
      // 합계주의(기간 겹침 → totals 중복 합산)를 반드시 같이 넘긴다.
      ? (promo && promo.promotions ? {
        합계: promo.totals, 기준: promo.기준 || null, 합계주의: promo.합계주의 || null,
        목록: promo.promotions.map((p) => ({
          몰: p.mall, 프로모션: p.name, 유형: p.유형 || null, 범위: p.범위 || null,
          프로모션기간: `${p.start}~${p.end}`, 집계기간: `${p.periodStart}~${p.periodEnd}`,
          목표매출: p.목표매출, 매출: p.sales, 주문: p.orders, 수량: p.qty,
        })),
      } : null)
      : NEED,
