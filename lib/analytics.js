...
async function inflowPaths(start, end) {
  ...
  const agg = (rows, keyName) => {
    ...
    return Object.entries(m).map(([k, visits]) => ({ [keyName]: k, visits })).sort((a, b) => b.visits - a.visits).slice(0, 20);
  };
...
async function behaviorStats(start, end) {
  ...
  const 페이지별뷰 = Object.values(pgMap).sort((a, b) => b.views - a.views).slice(0, 15);
  // 상품 조회수 × 장바구니 조인 → 퍼널
  const cartBy = {};
  ...
  }).sort((a, b) => b.조회수 - a.조회수).slice(0, 15);
  // 시간대별 구매 (0~23시 합산)
  const hrMap = {};
