...
async function dailyHealth(targetDate, weeks) {
  ...
  const [visitsArr, ordMap, dayArr] = await Promise.all([
    dailyVisitors(startD, d, 'total').catch(() => []),
    ordersByDate(startD, d),
    dailyVisitors(d, d, 'total').catch(() => []), // 타깃일 단독조회 = 메인 KPI(inflowReport)와 동일 방식
  ]);
  const vBy = Object.fromEntries(visitsArr.map((r) => [r.date, r.visits]));
  ...
  // 타깃일 방문수는 '단독 조회값' 우선. Cafe24 /visitors/view 는 진행중인 오늘에 한해
  //   범위조회(56일)와 단독조회 결과가 달라(예: 296 vs 301) 메인 KPI 방문수와 어긋난다.
  //   완료일은 단독=범위 동일(검증됨)하므로 안전하고, 과거 같은요일 평균(샘플)도 편향 없음.
  const dayRow = dayArr.find((r) => r.date === d) || {};
  const dayVisits = (dayRow.visits != null ? dayRow.visits : (vBy[d] || 0));
  const daySales = (ordMap[d] || {}).revenue || 0;
