...
async function behaviorStats(start, end) {
  ...
  const [pages, pViews, carts, times, uniq] = await Promise.all([
  ...
  const 상품퍼널 = Object.values(pvMap).map((x) => {
    ...
    return { ...x, 장바구니담기: adds, 담기율_pct: x.조회수 ? +((adds / x.조회수) * 100).toFixed(1) : null };
...
module.exports = { inflowReport, dailyVisitors, trafficDaily, trafficMonthly, dailyPV, dailySignups, dailyHealth, inflowPaths, behaviorStats };
