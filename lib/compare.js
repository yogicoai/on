...
async function periodCompare(start, end) {
  ...
  const channelRow = (ch) => {
    ...
    return {
      ...
      wow: { revenue: data.wow[ch].revenue, rate: rate(cur.revenue, data.wow[ch].revenue), orders: data.wow[ch].orders, ordersRate: rate(cur.orders, data.wow[ch].orders) },
      mom: { revenue: data.mom[ch].revenue, rate: rate(cur.revenue, data.mom[ch].revenue), orders: data.mom[ch].orders, ordersRate: rate(cur.orders, data.mom[ch].orders) },
      yoy: { revenue: data.yoy[ch].revenue, rate: rate(cur.revenue, data.yoy[ch].revenue), orders: data.yoy[ch].orders, ordersRate: rate(cur.orders, data.yoy[ch].orders) },
      aovCur: aov(cur),
...
// 특정 카테고리의 상품별 매출 (자사몰) — 카테고리 성과 드릴다운(팝업)용
async function categoryProducts(category, start, end) {
  const coll = await store.collection('orders_raw');
  const cur = coll.find({ order_date: { $gte: start, $lte: end }, paid: true, canceled: false }, { projection: { items: 1 } });
  const m = {};
  for await (const o of cur) for (const it of (o.items || [])) {
    if (categoryOf({}, it.product_no, it.product_name) !== category) continue;
    const k = String(it.product_name || '(미상)');
    const e = (m[k] = m[k] || { name: k, qty: 0, sales: 0 });
    e.qty += N(it.quantity); e.sales += N(it.payment_amount);
  }
  return { category, start, end, products: Object.values(m).map((r) => ({ name: r.name, qty: r.qty, sales: Math.round(r.sales) })).sort((a, b) => b.sales - a.sales) };
}
...
module.exports = { periodCompare, bestProducts, categoryPromo, categoryProducts, promoCompare, bothSum, productByChannel, tierByChannel, traffic, monthlySeries, monthlyTierSeries, growthTop, tierPeriods, refurb };
