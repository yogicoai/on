...
// 주문(embed=coupons)에서 직접 쿠폰명을 끌어와 order_coupons 에 적재.
//   /coupons API가 안 주는 '삭제 쿠폰'도 주문에 coupon_name 이 그대로 남아 있어 전부 캡처된다.
//   쿠폰 기준(coupon-driven)이 아니라 주문 기준(order-driven)이라 그 기간 사용 쿠폰을 빠짐없이 적재.
async function syncCouponNamesFromOrders(start, end, { onProgress } = {}) {
  const ocoll = await store.collection(OC_COLL);
  const oraw = await store.collection('orders_raw');
  const orders = await oraw.find({ order_date: { $gte: start, $lte: end }, paid: true, canceled: false, coupon_discount: { $gt: 0 } }, { projection: { order_id: 1 } }).toArray();
  const map = new Map(); let done = 0;
  await mapPool(orders, async (o) => {
    try {
      const r = await c.adminGet('/orders/' + o.order_id, { embed: 'coupons' });
      const cps = (r && r.order && r.order.coupons) || [];
      const set = new Set();
      for (const cp of cps) { const nm = String(cp.coupon_name || '').trim(); if (nm) set.add(nm); }
      if (set.size) map.set(o.order_id, set);
    } catch (_) {}
    done += 1; if (onProgress && done % 20 === 0) onProgress({ done, total: orders.length, mapped: map.size });
  }, 6);
  const now = new Date().toISOString();
  const ops = [];
  for (const [oid, set] of map) ops.push({ updateOne: { filter: { order_id: oid }, update: { $set: { order_id: oid, coupons: [...set], _syncedAt: now, _src: 'order' } }, upsert: true } });
  for (let i = 0; i < ops.length; i += 1000) { try { await ocoll.bulkWrite(ops.slice(i, i + 1000), { ordered: false }); } catch (_) {} }
  return { start, end, scanned: orders.length, mappedOrders: map.size };
}
...
module.exports = { productPromotion, couponFunnel, computeCouponFunnel, chunkRanges, fetchIssuesInRange, targetSummary, benefitSummary, mapPool, syncCouponNames, syncCouponNamesFromOrders, couponNamesFor };
