...
const promoDefs = require('./promoDefs'); // 프로모션 정의·목표(MD 제출 엑셀) — 구 promoTargets 대체
...
async function refreshTrafficDaily(fromDate) {
  ...
  const ops = [];
  let visitsMissing = 0, pvMissing = 0, signupsMissing = 0;
  for (const d of dates) {
    if (d >= today) continue; // 오늘은 미완료
    const o = ordMap[d] || {};
    const set = { date: d, orders: o.orders || 0, revenue: o.revenue || 0, _at: new Date().toISOString() };
    ...
    // 가입수도 동일하게 no-clobber. /customersprivacy 가 미완성 시점에 0을 주면 기존 가입수가 0으로 덮이던 버그 차단.
    //   (진짜 가입 0인 날은 insert 시 0으로 들어가고 이후엔 안 건드림. 자가치유는 trafficSeries 에서 보강)
    if (signupMap[d] > 0) set.signups = signupMap[d];
    else { onInsert.signups = 0; signupsMissing++; }
  ...
  if (signupsMissing) console.warn(`[refreshTrafficDaily] 가입수 미수집 ${signupsMissing}일 — 기존 값 보존(덮어쓰지 않음)`);
  return { from: fromDate, to: today, days: ops.length, visitsMissing, pvMissing, signupsMissing };
}
...
async function trafficSeries(fromDate) {
  ...
  try {
    ...
    const signupZero = rows.filter((r) => r.date >= lo && r.date < today && !r.signups);
    if (visitZero.length || pvZero.length || signupZero.length) {
      const [live, pvMap, suMap] = await Promise.all([
        ...
        signupZero.length ? analytics.dailySignups(lo, today).catch(() => ({})) : Promise.resolve({}),
      ]);
      ...
      for (const r of signupZero) { const s = suMap[r.date]; if (s > 0) { r.signups = s; (byDate[r.date] = byDate[r.date] || {}).signups = s; } }
      const ops = Object.entries(byDate).map(([date, fields]) => ({ updateOne: { filter: { date }, update: { $set: { ...fields, _at: new Date().toISOString() } } } }));
      if (ops.length) { await c.bulkWrite(ops, { ordered: false }); console.warn(`[trafficSeries] 방문/PV/가입 자가치유 ${ops.length}일`); }
    }
...
async function buildPromo(date) {
  ...
  // 프로모션 목록은 MD 제출 정의(promo_defs)에서 — 과거 promo_targets(입력 UI 폐지) 대체
  const [mdPromos, mdProducts, tgts] = await Promise.all([promoHistory.listPromos(), promoHistory.listProducts(), promoDefs.reportPromotions().catch(() => [])]);
  ...
  //    ① 오프라인·외부몰 전용은 정상가 마스터가 없어 역산 불가 → 자사몰/스마트스토어를 포함한 건만.
  //    ② scope='전제품'만. promoDiscountBreakdown 은 상품을 가리지 않고 기간 전체 할인액을 잡으므로,
  //       품목·제품군 프로모션(전제품 기간에 겹쳐 진행됨)까지 넣으면 같은 할인액이 프로모션마다 중복 계상된다.
  const ONLINE = ['자사몰', '스마트스토어'];
  const live = (tgts || []).filter((t) => t.start && t.end && t.start >= CUT && t.start <= date
    && t.scope === '전제품' && (t.malls || []).some((m) => ONLINE.includes(m)));
...
async function buildTargetConfig() {
  ...
  // 프로모션 목표: promo_defs.target_sales (MD가 엑셀에 넣은 값). 과거 promo_targets 수기입력 대체.
  //   트래픽 목표는 엑셀에 항목이 없어 항상 null → 리포트 트래픽 탭은 "트래픽 목표 미설정"으로 표시된다.
  let promotions = [];
  try { promotions = await promoDefs.reportPromotions(); } catch (_) {}
