...
async function refreshTrafficDaily(fromDate) {
  ...
  for (const d of dates) {
    ...
    const o = ordMap[d] || {};
    const set = { date: d, pv: pvMap[d] || 0, orders: o.orders || 0, revenue: o.revenue || 0, signups: signupMap[d] || 0, _at: new Date().toISOString() };
    ...
    else { update.$setOnInsert = { visits: 0 }; visitsMissing++; }
  ...
  return { from: fromDate, to: today, days: ops.length, visitsMissing };
}
...
async function trafficSeries(fromDate) {
  ...
  try {
    ...
    const zeroDays = rows.filter((r) => r.date >= lo && r.date < today && !r.visits);
    if (zeroDays.length) {
      const live = await analytics.dailyVisitors(lo, today, 'total').catch(() => []);
      const vBy = Object.fromEntries(live.map((x) => [x.date, x.visits]));
      const ops = [];
      for (const r of zeroDays) {
        const v = vBy[r.date];
        if (v > 0) { r.visits = v; ops.push({ updateOne: { filter: { date: r.date }, update: { $set: { visits: v, _at: new Date().toISOString() } } } }); }
      }
      if (ops.length) { await c.bulkWrite(ops, { ordered: false }); console.warn(`[trafficSeries] 방문수 자가치유 ${ops.length}일`); }
    }
...
// ── ds-promo (프로모션 성과 리뷰) — "과거 완료 프로모션" 전용 ──
//   2026-06 이전(이전 작업분) = MD 정본(promo_history, 할인율별 그대로). 이카운트엔 할인 식별값이 없어 재계산 불가 → import한 고정값.
//   2026-06~(우리 운영분)은 여기서 다루지 않는다. '전사 프로모션 목표 페이스' 테이블(renderPromotionTargetTable)이
//   promoTargets(팀 입력 목표) + 일별 series 실적으로 목표 대비 달성을 별도로 보여준다. (중복 집계 방지)
async function buildPromo() {
  const [mdPromos, mdProducts] = await Promise.all([promoHistory.listPromos(), promoHistory.listProducts()]);
  const promos = mdPromos.map((p) => ({ ...p, src: 'md' })).sort((a, b) => (a.Start < b.Start ? -1 : a.Start > b.Start ? 1 : 0));
  return { promos, products: mdProducts.map((x) => ({ ...x })) };
}
