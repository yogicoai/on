...
// ── 리포트 4개 데이터(ds-main/products/promo/traffic) 조립 ──
//   products/promo 는 우선 빈 구조(상품·프로모션 탭은 추후 채움) — 나머지 탭은 ds-main 으로 동작.
async function buildReport(date) {
  const [main, products, promo] = await Promise.all([
    buildMain(date),
    buildProducts('2025-01-01'),
    buildPromo(),
  ]);
  return { main, traffic: main.traffic, products, promo };
}

module.exports = { classifyChannel, CHANNELS, dailyChannelSeries, buildMain, buildReport, buildProducts, buildPromo, monthKPI, vendorDailyMap, monthCategories, dayPromo, trafficSeries, refreshTrafficDaily };
