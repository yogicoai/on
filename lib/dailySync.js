...
function commonRanges() {
  ...
  return [
    { label: '이번 달(MTD)', start: `${y}-${pad(m + 1)}-01`, end: today() },
    { label: '최근 30일', start: minusDays(29), end: today() },
    { label: '지난 달', start: fmt(new Date(y, m - 1, 1)), end: fmt(new Date(y, m, 0)) },
  ];
}
...
async function runToday({ log = console.log } = {}) {
  ...
  // 오늘 포함 구간만 빠르게 갱신 — funnel 은 강제하지 않음(캐시 유지 → 92초 스캔 회피)
  await warmRanges(commonRanges().filter((r) => r.end === d), { forceFunnel: false, log });
  log('[sync-today] 완료');
  return out;
}
