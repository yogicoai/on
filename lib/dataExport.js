...
async function fetchDataset(name, start, end) {
  const d = DATASETS[name];
  if (!d) throw new Error(`알 수 없는 dataset: ${name} — 사용 가능: ${Object.keys(DATASETS).join(', ')}`);
  if (d.range) {
    if (!/^\d{4}-\d{2}-\d{2}$/.test(start || '') || !/^\d{4}-\d{2}-\d{2}$/.test(end || '')) {
      throw new Error('start/end 필요 (YYYY-MM-DD)');
    }
    const rows = await d.fn(start, end);
    return { dataset: name, start, end, count: rows.length, rows, ...(rows.length >= MAX_ROWS ? { note: `상한 ${MAX_ROWS}행 — 기간을 좁혀 재요청하세요` } : {}) };
  }
  const r = await d.fn();
...
function catalog() {
  return {
    ...
    사용법: 'GET /api/export?dataset=<이름>&start=YYYY-MM-DD&end=YYYY-MM-DD&format=json|csv (헤더: Authorization: Bearer <EXPORT_TOKEN>)',
    주의: '고객 개인정보 미포함(집계 원장만). 실시간 아님 — 매출 매일 09:00, 광고 09:30, 재고 10분 주기 갱신.',
  };
}

module.exports = { fetchDataset, catalog, toCsv, DATASETS };
