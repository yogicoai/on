...
// 이카운트 온라인 매출 (on.orders) — 출고일 기준. 반품은 amount<0 음수 행 → isReturn=true로 표시(amount 합계=순매출)
async function salesOnline(start, end) {
  ...
  return rows.map((r) => ({ ...r, isReturn: Number(r.amount) < 0 }));
}

// 오프라인 매장 매출 (off.orders) — 반품은 amount<0 음수 행 → isReturn=true (amount 합계=순매출)
...
// 광고 캠페인 단위 (adboard.kv uploaded-campaigns) — 일별×캠페인
async function adsCampaign(start, end) {
  return adEfficiency.campaigns(start, end);
}
...
const DATASETS = {
  ...
  'ads-campaign': { fn: adsCampaign, range: true, desc: '광고 일별×캠페인 — 광고비·노출·클릭·전환·전환매출·장바구니 (CSV 업로드분, 7개 캠페인)' },
