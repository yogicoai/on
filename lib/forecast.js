...
function getJson(host, path) {
  return new Promise((resolve, reject) => {
    const req = https.get('https://' + host + path, (r) => {
      // 바이트(Buffer)를 모두 모은 뒤 한 번에 UTF-8 디코드 — 한글(3바이트)이 chunk 경계에서
      // 잘려 깨지는(�) 문제 방지. `b += c`(chunk마다 toString)는 경계 글자를 손상시킨다.
      const chunks = []; r.on('data', (c) => chunks.push(c));
      r.on('end', () => { try { resolve(JSON.parse(Buffer.concat(chunks).toString('utf8'))); } catch (e) { reject(new Error('API 응답 파싱 실패(' + host + ')')); } });
    });
    req.on('error', reject);
...
// 커버: 컬렉션(줄라/럭스 등)+색상 구분 → baseName(줄라 유지) + 색상.
const coverKey = (name, color) => baseName(name) + '|' + norm(stripSlash(color));
// 이너: 사이즈 기준. 줄라/럭스는 커버 컬렉션일 뿐이라 이너는 공통(맥스/줄라 맥스/럭스 맥스 → 맥스 이너).
//   단 롤·카터필러·디즈니 등 형태가 다른 라인은 자체 이너가 있어 떼지 않는다.
const INNER_COLLECTION = new Set(['줄라', '럭스', 'zola', 'luxe']);
const innerBase = (name) => norm(stripSlash(name).replace(/\(epp\)/gi, ' ').replace(/\bepp\b/gi, ' '))
  .split(' ').filter((t) => t && !BASE_STRIP.has(t.toLowerCase()) && !INNER_COLLECTION.has(t.toLowerCase())).join(' ');
// 인형(완구): 캐릭터가 판매는 이름·재고는 색상에 들어가 코드가 어긋남 → 이름+색상을 합쳐 토큰셋으로 매칭.
//   예) 판매 "티렉스 메이트 / 테디" ↔ 재고 "메이트 / 티렉스(테디)" — 둘 다 {메이트,티렉스,테디}.
const dollKey = (name, color) => {
  const s = (stripSlash(name) + ' ' + stripSlash(color)).replace(/[()]/g, ' ');
  return [...new Set(norm(s).split(' ').filter((t) => t && !BASE_STRIP.has(t.toLowerCase())))].sort().join(' ');
};
...
async function salesForecast({ months = 3, today, withCode = false } = {}) {
  ...
  for (const r of rows) {
    ...
    if (/케어서비스|수선비|박스\s*패키지|box\s*package/i.test(name)) continue; // A/S·수선비·포장재(박스)는 실판매 아님 → 제외
...
async function stockList() {
  ...
  return rows
    .filter((s) => s.category !== '제외' && !/박스\s*패키지|box\s*package/i.test(s.name || '')) // 포장재 제외
    .map((s) => {
      let name = s.name || '';
...
async function reorderPlan({ months = 3, targetMonths = 1, today } = {}) {
  ...
  const coverDemand = {}, innerDemand = {}, codeDemand = {}, dollDemand = {};
  const unmapped = [];
  for (const it of fc.items) {
    const nm = it.name || '';
    const isCover = nm.includes('커버'), isInner = nm.includes('이너');
    const isBody = !isCover && !isInner && (it.ecat === '소파' || it.ecat === '바디필로우' || it.ecat === '리퍼'); // 빈백 본품(리퍼브 재판매 포함)
    const ck = coverKey(nm, it.color); // 커버: 컬렉션+색상별
    const ik = innerBase(nm);          // 이너: 사이즈 기준(색상·컬렉션 무관, 전 색상 합산)
    ...
    if (!isCover && !isInner) { const dk = dollKey(nm, it.color); if (dk) dollDemand[dk] = (dollDemand[dk] || 0) + it.monthlyAvg; } // 인형 이름+색상 토큰셋 매칭용
  }
  ...
  for (const s of stock) {
    if (s.category === '커버' || s.category === '이너') {
      ...
      const bk = isInnerCat ? innerBase(s.name) : coverKey(s.name, s.color); // 이너=사이즈, 커버=컬렉션+색상
      const gk = s.category + '|' + bk;
      const g = groups[gk] = groups[gk] || { kind: s.category, baseKey: bk, name: (isInnerCat ? innerBase(s.name) : baseName(s.name)) + ' ' + s.category, color: isInnerCat ? '' : norm(stripSlash(s.color)), category: s.category, codes: [], stock: 0 };
  ...
  for (const s of others) {
    let demand = codeDemand[s.code] || 0, basis = '품목코드';
    if (demand === 0 && s.category === '인형') { // 인형은 코드 미매칭 시 이름+색상 토큰셋으로 보강
      const d = dollDemand[dollKey(s.name, s.color)] || 0;
      if (d > 0) { demand = d; basis = '인형(이름+색상)'; }
    }
    const monthlyAvg = round1(demand);
    const { monthsLeft, needOrder, suggestQty } = reorderCalc(s.qty, monthlyAvg, targetMonths);
    items.push({ code: s.code, variants: 1, category: s.category, name: s.name, color: s.color, stock: s.qty, monthlyAvg, expected1m: monthlyAvg, monthsLeft, needOrder, suggestQty, matched: monthlyAvg > 0, basis });
  }
  return {
