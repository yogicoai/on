...
const offline = require('./offline'); // 오프라인 매장(off.orders) — 광고=온라인=오프라인 3축 연결용
...
async function overview(start, end) {
  ...
  const [ad, series, traffic, offDaily] = await Promise.all([
    ...
    offline.dailySeries(start, end).catch(() => []),
  ]);
  ...
  const 오프라인매출 = offDaily.reduce((a, d) => a + (d.매출 || 0), 0);
  const 전사매출 = 실매출 + 오프라인매출;
  ...
  return {
    ...
    매출: {
      ...
      오프라인_매장: R(오프라인매출),
      오프라인비중_pct: 전사매출 ? +((오프라인매출 / 전사매출) * 100).toFixed(1) : null,
    },
    광고: {
    ...
    교차지표: {
      실질_마케팅비용률_pct: 비용률,        // 광고비 ÷ 온라인 실매출
      전사기준_비용률_pct: 전사매출 ? +((spend / 전사매출) * 100).toFixed(2) : null, // 광고비 ÷ (온라인+오프라인)
...
async function series(start, end) {
  ...
  const [adRows, channel, traffic, memberDaily, offDaily] = await Promise.all([
  ...
  const rows = enumDays(start, end).map((date) => {
    ...
    const 온라인 = 자사몰 + 스토어 + 외부; const 오프라인 = R(offBy[date] || 0);
    return {
      date,
      광고: { 네이버: R(ad.네이버), 메타: R(ad.메타), 크리테오: R(ad.크리테오), 카카오: R(ad.카카오), 합계: R(ad._spend), 전환매출: R(ad._cv) },
      매출: { 자사몰, 스마트스토어: 스토어, 외부채널: 외부, 온라인합계: 온라인, 오프라인_매장: 오프라인, 전사: 온라인 + 오프라인 },
...
function totalsOf(rows) {
  const t = { 광고비: 0, 광고_네이버: 0, 광고_메타: 0, 광고_크리테오: 0, 광고_카카오: 0, 광고전환매출: 0, 온라인매출: 0, 자사몰: 0, 스마트스토어: 0, 외부채널: 0, 오프라인매출: 0, 전사매출: 0, 회원매출: 0, 비회원매출: 0, 방문: 0, 신규가입: 0, 구매: 0 };
  for (const r of rows) {
    t.광고비 += r.광고.합계; t.광고_네이버 += r.광고.네이버; t.광고_메타 += r.광고.메타; t.광고_크리테오 += r.광고.크리테오; t.광고_카카오 += r.광고.카카오; t.광고전환매출 += r.광고.전환매출;
    t.온라인매출 += r.매출.온라인합계; t.자사몰 += r.매출.자사몰; t.스마트스토어 += r.매출.스마트스토어; t.외부채널 += r.매출.외부채널;
    t.오프라인매출 += r.매출.오프라인_매장; t.전사매출 += r.매출.전사;
    t.회원매출 += r.자사몰_회원매출; t.비회원매출 += r.자사몰_비회원매출;
    t.방문 += r.방문; t.신규가입 += r.신규가입; t.구매 += r.구매;
  }
  t.마케팅비용률_pct = t.온라인매출 ? +((t.광고비 / t.온라인매출) * 100).toFixed(2) : null;
  t.전사기준_비용률_pct = t.전사매출 ? +((t.광고비 / t.전사매출) * 100).toFixed(2) : null;
...
//   방향 규약: A = 비교 기준(평소·직전·전년), B = 관심 구간(프로모션·광고 늘린 주).
//   증감률은 (B-A)/A = "A에서 B로 얼마나 변했나". A·B를 거꾸로 넣으면 부호가 뒤집혀 읽히므로
//   키 이름에 방향을 박아 둔다(구 이름 '증감_pct'는 방향이 없어 프로모션을 실패로 읽는 오독을 낳았다).
async function periodCompare(aStart, aEnd, bStart, bEnd) {
  ...
  const keys = ['광고비', '온라인매출', '자사몰', '스마트스토어', '외부채널', '오프라인매출', '전사매출', '회원매출', '비회원매출', '방문', '신규가입', '구매'];
  const 일평균비교 = {};
  for (const k of keys) {
    const av = ta[k] / daysA, bv = tb[k] / daysB;
    일평균비교[k] = { A: R(av), B: R(bv), A대비B_증감_pct: av ? +(((bv - av) / av) * 100).toFixed(1) : null };
  }
  const dSpend = (tb.광고비 / daysB) - (ta.광고비 / daysA);
  const dSales = (tb.온라인매출 / daysB) - (ta.온라인매출 / daysA);
  return {
    A: { start: aStart, end: aEnd, days: daysA, ...ta },
    B: { start: bStart, end: bEnd, days: daysB, ...tb },
    일평균비교,
    // (B-A 일평균 매출증분) ÷ (광고비증분). 광고비가 줄어든 구간에선 음수÷음수로 큰 양수가 나와
    //   "광고를 줄였는데 효율이 폭등"처럼 읽힌다 → 광고비가 늘어난 경우에만 값을 낸다.
    증분ROAS: dSpend > 0 ? +(dSales / dSpend).toFixed(2) : null,
    ...(dSpend > 0 ? {} : { 증분ROAS_미산출: `B구간 광고비가 A보다 일평균 ${R(-dSpend)}원 ${dSpend === 0 ? '동일' : '적음'} — 증분ROAS는 광고비를 늘린 구간에서만 의미가 있어 산출하지 않았습니다.` }),
    방향: 'A=비교 기준(평소·직전·전년), B=관심 구간(프로모션·광고 늘린 주). A대비B_증감_pct 와 증분ROAS 는 모두 "A→B 변화" — A·B를 거꾸로 넣으면 부호가 반대로 읽힌다.',
