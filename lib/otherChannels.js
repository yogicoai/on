...
async function groupsPeriodCompare(start, end) {
  ...
  const rows = (ov.groups || []).map((g) => ({
    group: g.group,
    cur: { revenue: g.sales, aov: g.aov || 0, orders: g.orders || 0 },
    wow: { rate: rate(g.sales, wow[g.group] || 0) },
    mom: { rate: rate(g.sales, mom[g.group] || 0) },
    yoy: { rate: rate(g.sales, yoy[g.group] || 0) },
  }));
