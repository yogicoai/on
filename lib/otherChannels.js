...
// 자사몰·스마트스토어는 API로 별도 수집 → 기타 채널에서 제외. 더현대닷컴·신세계대구·현대미아·현대H몰(홈쇼핑)은 채널에서 제외(요청).
//   현대는 '현대 이지웰' / '현대 M포인트몰' 두 입점몰만 분리 표기하고, 나머지 소규모 현대 입점은 제외.
const EXCLUDE = ['홈페이지', '스마트스토어', '더현대닷컴', '신세계대구(온라인몰/본사운영)', '현대미아(온라인몰/본사운영)', '현대H몰(현대홈쇼핑)'];
...
const GROUP_RULES = [
  ...
  // 현대는 입점몰별로 분리(합산 안 함): 현대 이지웰 / 현대 M포인트몰 각각. (나머지 현대 입점은 EXCLUDE 처리)
  { group: '현대 이지웰',     test: (s) => s.includes('이지웰') },                         // 현대이지웰
  { group: '현대 M포인트몰',  test: (s) => /m\s*포인트/i.test(s) || s.includes('엠포인트') }, // 현대M포인트몰
...
// 특정 store(홈페이지=자사몰, 스마트스토어 등)의 이카운트 매출(출고 기준). EXCLUDE 무시(직접 지정).
//   latestDate: 그 store의 최신 적재일 — 최근 구간이 비면 '적재 지연(미출고)' 안내용.
async function storeRevenue(storeName, start, end) {
  const c = await coll();
  const m = { store: storeName };
  if (start || end) { m.date = {}; if (start) m.date.$gte = start; if (end) m.date.$lte = end; }
  const [agg, latest] = await Promise.all([
    c.aggregate([{ $match: m }, { $group: { _id: null, amount: { $sum: '$amount' }, qty: { $sum: '$qty' }, rows: { $sum: 1 } } }]).toArray(),
    c.find({ store: storeName, date: { $nin: ['', '-', '날짜오류'] } }, { projection: { date: 1 } }).sort({ date: -1 }).limit(1).toArray(),
  ]);
  const r = agg[0] || {};
  return { store: storeName, start: start || null, end: end || null, amount: Math.round(r.amount || 0), qty: r.qty || 0, rows: r.rows || 0, latestDate: (latest[0] || {}).date || null };
}

module.exports = { overview, channelDetail, groupDetail, groupBreakdownProducts, groupOf, groupTotal, groupsPeriodCompare, totalForRange, storeRevenue, EXCLUDE };
