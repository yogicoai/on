...
// ── 프로모션 기간 할인율별 성과 — 정상가 vs 실판매가로 할인율 역산 후 5%단위 티어로 버킷 ──
//   자사몰(Cafe24): product_prices(cafe24, 역산 정상가) 대비 실판매단가(payment_amount/qty)
//   스토어(SmartStore): product_amount(정상가, 데이터에 직접 존재) 대비 payment_amount(실판매)
//   정가(≈0%)·이상치(>40%, 정상가 오류)는 프로모션 성과에서 제외, 5~30% 할인분만 집계(=프로모션 할인 발생분).
//   ※ 주문일 기준(Cafe24/Naver 주문) — 이카운트(출고일) 매출과 기준이 다를 수 있음.
const PROMO_TIERS = [5, 10, 15, 20, 25, 30];
function snapPromoTier(pct) {
  if (pct < 2.5) return 0;   // 정가(할인 없음)
  if (pct > 40) return -1;   // 이상치(정상가 오류/특수)
  return PROMO_TIERS.reduce((best, t) => (Math.abs(t - pct) < Math.abs(best - pct) ? t : best), PROMO_TIERS[0]);
}
async function promoDiscountBreakdown(start, end) {
  const tiers = {};
  const T = (r) => (tiers[r] = tiers[r] || { jaSales: 0, jaQty: 0, jaOrd: new Set(), ssSales: 0, ssQty: 0, ssOrd: new Set() });
  const ex = { 정가: { ja: 0, ss: 0 }, 이상치: { ja: 0, ss: 0 }, 미등록: { ja: 0, ss: 0 } };
  let jaTotal = 0, ssTotal = 0;
  // 1) 자사몰 — Cafe24 orders_raw 라인별
  const prices = await priceMap('cafe24');
  const oc = await store.collection('orders_raw');
  const jaRows = await oc.aggregate([
    { $match: { paid: true, canceled: false, order_date: { $gte: start, $lte: end } } },
    { $unwind: '$items' },
    { $match: { 'items.quantity': { $gt: 0 }, 'items.payment_amount': { $gt: 0 } } },
    { $project: { order_id: 1, pno: '$items.product_no', qty: '$items.quantity', sales: '$items.payment_amount' } },
  ]).toArray();
  for (const r of jaRows) {
    jaTotal += r.sales;
    const p = prices[String(r.pno)];
    if (!p || !p.normal_price) { ex.미등록.ja += r.sales; continue; }
    const unit = r.qty ? r.sales / r.qty : 0;
    const tier = snapPromoTier((1 - unit / p.normal_price) * 100);
    if (tier === 0) { ex.정가.ja += r.sales; continue; }
    if (tier < 0) { ex.이상치.ja += r.sales; continue; }
    const t = T(tier); t.jaSales += r.sales; t.jaQty += r.qty; t.jaOrd.add(r.order_id);
  }
  // 2) 스토어 — SmartStore product_amount(정상가) vs payment_amount(실판매)
  const sc = await store.collection('smartstore_orders');
  const ssRows = await sc.find({ canceled: false, order_date: { $gte: start, $lte: end }, payment_amount: { $gt: 0 }, product_amount: { $gt: 0 } },
    { projection: { order_id: 1, quantity: 1, payment_amount: 1, product_amount: 1 } }).toArray();
  for (const r of ssRows) {
    ssTotal += r.payment_amount;
    const tier = snapPromoTier((1 - r.payment_amount / r.product_amount) * 100);
    if (tier === 0) { ex.정가.ss += r.payment_amount; continue; }
    if (tier < 0) { ex.이상치.ss += r.payment_amount; continue; }
    const t = T(tier); t.ssSales += r.payment_amount; t.ssQty += (r.quantity || 1); t.ssOrd.add(r.order_id);
  }
  const out = Object.keys(tiers).map(Number).sort((a, b) => a - b).map((rate) => {
    const t = tiers[rate];
    return { rate, JaAmount: Math.round(t.jaSales), JaQty: t.jaQty, JaOrders: t.jaOrd.size,
      SsAmount: Math.round(t.ssSales), SsQty: t.ssQty, SsOrders: t.ssOrd.size,
      Amount: Math.round(t.jaSales + t.ssSales), Orders: t.jaOrd.size + t.ssOrd.size };
  });
  const promoJa = out.reduce((a, x) => a + x.JaAmount, 0), promoSs = out.reduce((a, x) => a + x.SsAmount, 0);
  return {
    start, end, tiers: out,
    excluded: { 정가: { ja: Math.round(ex.정가.ja), ss: Math.round(ex.정가.ss) }, 이상치: { ja: Math.round(ex.이상치.ja), ss: Math.round(ex.이상치.ss) }, 미등록: { ja: Math.round(ex.미등록.ja), ss: Math.round(ex.미등록.ss) } },
    totals: { jaSales: Math.round(jaTotal), ssSales: Math.round(ssTotal), promoJa, promoSs },
  };
}

// 일별 "할인 발생 매출"(자사몰+스토어) — 당일 KPI 카드용. promoDiscountBreakdown 과 같은 기준(정가·이상치 제외).
async function promoDiscountDaily(start, end) {
  const out = {};
  const D = (d) => (out[d] = out[d] || { ja: 0, ss: 0, jaOrd: new Set(), ssOrd: new Set() });
  const prices = await priceMap('cafe24');
  const oc = await store.collection('orders_raw');
  const jaRows = await oc.aggregate([
    { $match: { paid: true, canceled: false, order_date: { $gte: start, $lte: end } } },
    { $unwind: '$items' },
    { $match: { 'items.quantity': { $gt: 0 }, 'items.payment_amount': { $gt: 0 } } },
    { $project: { date: '$order_date', oid: '$order_id', pno: '$items.product_no', qty: '$items.quantity', sales: '$items.payment_amount' } },
  ]).toArray();
  for (const r of jaRows) {
    const p = prices[String(r.pno)]; if (!p || !p.normal_price) continue;
    if (snapPromoTier((1 - (r.sales / r.qty) / p.normal_price) * 100) > 0) { const d = D(r.date); d.ja += r.sales; d.jaOrd.add(r.oid); }
  }
  const sc = await store.collection('smartstore_orders');
  const ssRows = await sc.find({ canceled: false, order_date: { $gte: start, $lte: end }, payment_amount: { $gt: 0 }, product_amount: { $gt: 0 } },
    { projection: { order_date: 1, order_id: 1, payment_amount: 1, product_amount: 1 } }).toArray();
  for (const r of ssRows) {
    if (snapPromoTier((1 - r.payment_amount / r.product_amount) * 100) > 0) { const d = D(r.order_date); d.ss += r.payment_amount; d.ssOrd.add(r.order_id); }
  }
  const res = {};
  for (const [date, v] of Object.entries(out)) res[date] = { 자사몰: Math.round(v.ja), 스마트스토어: Math.round(v.ss), jaOrders: v.jaOrd.size, ssOrders: v.ssOrd.size };
  return res;
}

module.exports = { upsert, list, priceMap, deriveFromOrders, discountAnalysis, promoDiscountBreakdown, promoDiscountDaily, coll };
