...
async function searchCatalog(q, source, limit, offset) {
  ...
  try {
    ...
    // 오래된 등록순(product_no 오름차순). productNo가 문자열이라 숫자로 변환해 정렬.
    const arr = await c.aggregate([
      { $match: m },
      { $addFields: { _pn: { $convert: { input: '$productNo', to: 'long', onError: Number.MAX_SAFE_INTEGER, onNull: Number.MAX_SAFE_INTEGER } } } },
      { $sort: { _pn: 1, updatedAt: 1 } },
