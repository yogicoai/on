...
async function searchSmartstore(q, limit) {
  if (!smartstore.enabled()) throw new Error('스마트스토어 토큰 미설정');
  const size = Math.min(Number(limit) || 50, 100);
  const body = { searchKeyword: (q || '').trim(), productStatusTypes: ['SALE'], page: 1, size };
...
async function searchCatalog(q, source, limit) {
  if (!store.configured()) return [];
  try {
    const c = await store.collection(CAT);
    const m = { source };
    if (q && q.trim()) m.productName = { $regex: q.trim().replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), $options: 'i' };
    const arr = await c.find(m).sort({ updatedAt: -1 }).limit(Math.min(Number(limit) || 50, 100)).toArray();
    return arr.map((p) => ({ productNo: p.productNo, productName: p.productName, price: p.price, source }));
  } catch (_) { return []; }
}

async function search(q, mall, limit) {
  const source = (mall === '스마트스토어' || mall === 'smartstore') ? 'smartstore' : 'cafe24';
  try {
    let live = source === 'smartstore' ? await searchSmartstore(q, limit) : await cafe24Products.search(q, limit);
    live = live.map((p) => ({ ...p, source }));
    await accumulate(live, source);
    return { source, items: live };
  } catch (e) {
    const cat = await searchCatalog(q, source, limit);
