...
async function allForPeriod(start, end) {
  ...
  const out = { start: start || null, end: end || null, totals, promotions: rows };
  // 빈 결과는 "조회 실패"가 아니라 "등록된 프로모션 없음"임을 명시 — LLM이 오해하지 않도록.
  if (!rows.length) out.note = ranged ? '이 기간에 등록된(겹치는) 프로모션이 없습니다 — 프로모션 성과가 아니라 등록 자체가 없음.' : '등록된 프로모션이 없습니다.';
  return out;
}
