...
function clean(b) {
  ...
  return {
    ...
    // 전사(자사몰+스토어에서 할인 진행) | 외부몰(쿠팡·롯데홈쇼핑 등 입점몰 전용) — 프로모션 성과 자동집계 대상 구분
    scope: (b.scope === '외부몰' ? '외부몰' : '전사'),
...
async function listTargets() {
  ...
  try {
    ...
    return arr.map((d) => ({ id: String(d._id), name: d.name, start: d.start, end: d.end, scope: d.scope || '전사', channels: d.channels || {}, trafficTargets: d.trafficTargets || {}, updatedAt: d.updatedAt }));
  } catch (_) { return []; }
}
