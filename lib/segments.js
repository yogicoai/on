...
//  결과를 bizpromote_cache 에 저장 → 주문 데이터가 새로 안 들어왔으면(지문 동일) 이전 결과를 바로 반환(force로 강제 재계산).
async function bizPromote(months = 3, { withPII = true, limit = 3000, force = false } = {}) {
  ...
  if (!force && withPII && cacheColl) {
    ...
    if (hit && hit.fingerprint === fp) {
      return { months, count: hit.count, rows: (hit.rows || []).slice(0, limit), cached: true, builtAt: hit.builtAt };
    }
  }
  ...
  return { ...out, cached: false, builtAt };
}
