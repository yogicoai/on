...
const BIZ_PROMO_MAIN = new Set(['소파', '바디필로우']); // 비즈 유도 대상 본품: 소파·바디필로우 구매자만(메이트 제외)
...
async function bizPromote(months = 3, { limit = 3000 } = {}) {
  ...
  for await (const o of cursor) {
    ...
    if (BIZ_PROMO_MAIN.has(cat)) { if (!p.mainDate || o.order_date > p.mainDate) p.mainDate = o.order_date; p.mainProducts.add(cleanName(o.product_name) || o.product_name); } // 소파·바디필로우 구매자만
