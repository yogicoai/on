...
// 같은 클러스터의 다른 DB 컬렉션 접근 (이카운트 'on' DB 등) — 동일 커넥션 재사용
async function namedCollection(dbName, collName) {
  await db(); // 커넥션 보장
  return _g.__onlinedata.client.db(dbName).collection(collName);
}
...
module.exports = { configured, collection, namedCollection, getCache, putCache, listCache, deleteOverlapping, close, DB_NAME };
