...
async function _run(j) {
  ...
  try {
    if (j.task === 'today') {
      ...
      // 버튼 트리거: 쿠폰 funnel 풀스캔(구간당 ~90초) 생략 → 주문/트래픽만 빠르게 재집계(수십 초).
      //   쿠폰 funnel 은 야간 스케줄러(runDaily funnel:true)가 매일 갱신하므로 캐시 유지로 충분.
      j.result = await dailySync.runDaily({ days: Math.min(Number(j.params.days) || 7, 31), funnel: false, log });
